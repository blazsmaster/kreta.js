...
/**
 * @description Készít egy véletlenszerű sértést a Kréta DirtyWords.xml fájl alapján
...
export const createKretaInsult = (prefix?: string) => {
	...
	const a = txts.filter((txt: any) => txt.type === 1);
	const n = txts.filter((txt: any) => txt.type === 2);
