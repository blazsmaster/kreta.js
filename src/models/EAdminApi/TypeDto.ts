...
export default class TypeDto implements Partial<TypeFields> {
	@IsString()
	...
	private validationErrorResponse(errors: Array<ValidationError>): object {
		const validFields: Partial<TypeFields> = {
			kod: this.code,
			leiras: this.description,
			azonosito: this.id,
			nev: this.name,
		...
		for (const error of errors) {
			validFields[error.property as keyof TypeFields] = undefined;
